...
# -------------------------------------------------------------------
# API base URL (inside docker-compose, service name = api)
# -------------------------------------------------------------------
    ...
    with st.container():
        ...
        with cols[0]:
            ...
            if st.session_state.user_email:
                st.markdown(f"<span style='font-size:12px;color:#e5e7ff;'>Signed in as {st.session_state.user_email}</span>",
                            unsafe_allow_html=True)
...
def auth_page():
    ...
    with center:
        ...
        st.markdown(
            "<div style='text-align:center;color:#ffffff;font-size:20px;font-weight:600;margin-bottom:20px;'>Dashboard Login</div>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown("<div style='text-align:center;margin-top:10px;'><span style='color:#e5e7ff;font-size:13px;'>Forgot password?</span></div>",
                    unsafe_allow_html=True)
        st.markdown("<div style='text-align:center;margin-top:16px;'><span style='color:#e5e7ff;font-size:13px;'>Don't have an account?</span></div>",
                    unsafe_allow_html=True)
    ...
    if st.session_state.show_register:
        ...
        with r_center:
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.markdown(
                "<div style='text-align:center;color:#ffffff;font-size:18px;font-weight:600;margin-bottom:18px;'>Create Account</div>",
                unsafe_allow_html=True,
            )

            reg_email = st.text_input("", placeholder="EMAIL", label_visibility="collapsed", key="reg_email")
            reg_password = st.text_input("", placeholder="PASSWORD", type="password",
                                         label_visibility="collapsed", key="reg_password")
            reg_confirm = st.text_input("", placeholder="CONFIRM PASSWORD", type="password",
...
def run_algorithm_for_project(project_id, strategy="gaussian", create_experiment=True):
    try:
        r = requests.post(
...
def dashboard_page():
    ...
    with top_cols[0]:
        st.markdown(
            "<div class='metric-card'><div class='metric-label'>Total Revenue</div>"
            f"<div class='metric-value'>$ {total_reward:,.2f}</div></div>",
            unsafe_allow_html=True,
        )
    with top_cols[1]:
        st.markdown("<div class='metric-card'><div class='metric-label'>Average Conversion</div>"
                    "<div class='metric-value'>22%</div></div>", unsafe_allow_html=True)
    with top_cols[2]:
        st.markdown("<div class='metric-card'><div class='metric-label'>Regret Minimization</div>"
                    "<div class='metric-value'>5%</div></div>", unsafe_allow_html=True)
    with top_cols[3]:
    ...
    st.markdown("<div class='subheading' style='margin-top:20px;'>Revenue And Trend Over Time</div>",
                unsafe_allow_html=True)
...
def product_details_page():
    ...
    selected_id = st.selectbox("Select Product", project_ids,
                               index=0,
    ...
    with col_right:
        ...
        if st.button("Run Algorithm", use_container_width=True):
            res = run_algorithm_for_project(selected_id, strategy="gaussian", create_experiment=True)
            if res:
                st.success(
                    f"Thompson Sampling selected price ${float(res['optimal_price']):.2f} for project {selected_id}."
                )
                st.rerun()
            else:
    ...
    st.markdown("<div class='subheading' style='margin-top:20px;'>Active Experiments Table</div>",
                unsafe_allow_html=True)
...
def add_product_page():
    ...
    with st.form("add_product_form"):
        ...
        product_description = st.text_area("", placeholder="PRODUCT DESCRIPTION",
    ...
    if start_clicked:
        ...
        try:
            ...
            res = run_algorithm_for_project(project_id, strategy="gaussian", create_experiment=True)
