...
if page == "Dashboard":
    ...
    st.markdown("<div style='float:right;margin-top:-70px;'>", unsafe_allow_html=True)
    st.button("CREATE NEW EXPERIMENT")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.table({"Product Name":[],"Current Best Price":[],"Conversion Rate":[],"Revenue":[],"Status":[]})

    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Revenue And Trend Trend Over Time</div>', unsafe_allow_html=True)
...
if page == "Product Status":
    ...
    st.markdown('<div class="section-title">Active Experiments Table</div>', unsafe_allow_html=True)
    st.table({"Price Variant":[],"Impressions":[],"Conversion Rate":[],"Revenue":[],"Allocated Traffic":[]})
